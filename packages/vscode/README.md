...
2. Open the **Observatory Traces** view in the Activity Bar (the microscope icon, badged with the pending-edit count). As Claude edits files, they appear grouped by folder → file → class.
...
- **Edits · Diffs · File History** (Observatory Traces, Activity Bar) — pending edits grouped **folder → file → class**, a diff tree, and the active file's history. In the editor, each edit gets a **✨ gutter star**, a subtle green/red line tint, a coral ruler mark, and an inline **🔬 #N · +A −R · n/m** menu (**✓ Keep · ↩ Undo · 💬 Chat · ⧉ Diff · ⋯ Details**).
- **The floating review bar** — while the open file has edits awaiting review, a compact bar floats over the code at the current edit: a live **Claude edit #N · +A −R · Diff n/m · File i/k** title with **Keep · Undo · ⌃⌄ · ‹› · ⧉ Diff · ⋯ Details** beside it. It follows as you keep, undo and step, and never steals focus. **⋯ Details** swaps it for the full **review bubble** (Claude's reasoning + the diff in git's own colors); **Collapse** returns. `claudeObservatory.editorReviewSurface` picks the bar, the bubble, or neither.

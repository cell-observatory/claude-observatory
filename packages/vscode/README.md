...
- **Edits · Diffs · File History** (Observatory Traces, Activity Bar) — pending edits grouped **folder → file → class**, a diff tree, and the active file's history. In the editor, each edit gets a **✨ gutter star**, a subtle green/red line tint, a coral ruler mark, and an inline **✦ #N · view changes** menu that opens an **inline review bubble** in git's own colors (reasoning + `+A −R`, with Keep/Undo/Chat/Prev/Next buttons).

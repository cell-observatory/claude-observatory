...
echo
echo "  ! This file carries LOCAL fixes that upstream does not have yet — a sync silently drops them:"
echo "      • the statusLine command is QUOTED (a config dir with a space, i.e. every Windows box"
echo "        with a spaced username, otherwise reaches Claude Code as two arguments)"
echo "      • the jq error names a Windows install route"
echo "    \`npm test\` fails if either is missing. Re-apply them, or land them upstream first."

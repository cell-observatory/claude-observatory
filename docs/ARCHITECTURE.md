...
| `views [--views <a,b,c>] [--root <d>]` | `{ [name]: payload \| null }` over the READ-ONLY views — default `changemap, multitask, prompts, processes, sessions, observations, risk, egress`. Each payload is **byte-identical** to that view's own command (pinned by e2e); a view that throws is `null` rather than fatal to the batch, and a mutating verb is refused outright. One process instead of one per view | The JetBrains refresh tick in full — `ObservatoryCli.viewBatch`; a client with an in-process core (VS Code) has no use for it |
